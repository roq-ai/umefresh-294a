...
import {
  ...
  useShippingAddressCount,
  useBillingInformationCount,
  useProductCount,
} from 'lib/roq';

import { BillingInformationListPage as ShowCaseList } from 'pages/billing-informations';

function DashboardPage() {
  const { session } = useSession();

  const shippingAddressStat = useShippingAddressCount();

  const billingInformationStat = useBillingInformationCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: shippingAddressStat?.data,
      isLoading: shippingAddressStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Billing Informations',
      result: billingInformationStat?.data,
      isLoading: billingInformationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Products',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Billing Informations
                </Text>
                <NextLink href={`billing-informations`} passHref legacyBehavior>
                  <Button
